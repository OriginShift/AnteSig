...
| P1 | Production dependency advisories remain open | `ASDF_NODEJS_VERSION=22.23.1 pnpm audit --prod --audit-level=moderate` on this checkout reports 6 vulnerabilities: 2 moderate and 4 high, in the transitive `sharp`, `postcss`, and `nanoid` paths. | Do not claim a clean dependency audit. Re-run the audit after dependency remediation and record the exact result before a release tag. |
